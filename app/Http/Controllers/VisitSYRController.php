...
class VisitSYRController extends Controller
{
     ...
     * @param $arr
     * @return VisitSYR|bool|\Illuminate\Database\Eloquent\Model
     */
...
, (
      ...
      FROM [dbo].[V_HH_VisitDuration] as visit
      INNER JOIN dbo.HH_VisitVerification as verify ON visit.ID = verify.VisitNo
      WHERE visit.starttime >= ? AND visit.starttime < ?  AND visit.SalesmanNo like 'SYR%'
        ", [$startDate , $endDate]);
    ...
    private function check_duplicate($visit_id){
        $exist = DB::select('SELECT visit_id FROM GoogleVisitsSYR WHERE visit_id = ? ',[$visit_id]);
