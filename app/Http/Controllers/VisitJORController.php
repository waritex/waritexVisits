...
class VisitJORController extends Controller
{
     ...
     * @param $arr
     * @return VisitJOR|bool|\Illuminate\Database\Eloquent\Model
     */
    private function create($arr){
        ...
        return VisitJOR::create($data);
        //return DB::insert('insert into GoogleVisitsJOR (id, visit_id , visit_start , visit_finish, current_customer_lat, , current_customer_lng) values (?, ?)', [1, 'Dayle']);
    }
...
, (
      ...
      FROM [dbo].[V_HH_VisitDuration] as visit
      INNER JOIN dbo.HH_VisitVerification as verify ON visit.ID = verify.VisitNo
      WHERE visit.starttime >= ? AND visit.starttime < ?  AND visit.SalesmanNo like 'JOR%'
        ", [$startDate , $endDate]);
    ...
    private function check_duplicate($visit_id){
        $exist = DB::select('SELECT visit_id FROM GoogleVisitsJOR WHERE visit_id = ? ',[$visit_id]);
